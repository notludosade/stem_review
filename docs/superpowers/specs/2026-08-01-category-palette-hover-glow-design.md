...
**Cascade priority** (declared in this order in `style.css`, later rules win on equal specificity — same mechanism the site already relies on for tier vs. pathway): `data-tier` (base) → `data-category` (overrides tier) → `data-pathway` (overrides both, most specific).

This order was corrected after checking the actual markup: the Sandbox hub pages (`python-sandbox.html`, `java-sandbox.html`, etc.) already carry `data-tier="intro"` on their top-level `.page` div as an incidental default (sandboxes aren't meaningfully "intro vs. advanced" content — every sandbox page has the same tier value). If tier outranked category, adding `data-category="sandbox"` to those same elements would have no visible effect — the intro-tier teal would keep winning, silently defeating the goal of giving Sandbox its own identity on exactly the pages meant to show it. Category outranking tier fixes this, and doesn't affect anything else: pathway pages (`Projects/*.html`, `Pathways/*.html`) never carry `data-tier` on the same element, so pathway color is unaffected; Subject/lesson pages never receive `data-category` in this sub-project (Subject has no override, see below), so ordinary lesson tier coloring is unaffected.
