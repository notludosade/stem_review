...
The final review (base `9bd78e4`, head `89025e4`) found the implementation of Tasks 1–8 individually correct, but surfaced a gap in the plan itself plus two Important defects from the `assets/` relocation. All three are fixed in one round, mechanically, with no change to any task's already-approved design:

**Fix 1 (Critical, plan gap, not an implementation error):** Task 1 moved only `index.html` into `content/`. Adding a `build` script (Task 1) makes Vercel switch from zero-config static hosting to the Next.js preset, which serves only `.next` output plus `public/` — so the other 22 root `.html` files and ~1,460 files across the repo's 33 course directories (1,482 files total, verified via `find . -name "*.html" -not -path "./content/*" -not -path "./public/*" -not -path "./node_modules/*" -not -path "./.next/*" | wc -l`) would 404 in production, including every link in the new shell's nav. Fix: `git mv` all 22 root `.html` files and all 33 course directories into `public/`, unchanged, so Vercel keeps serving them exactly as before — `public/` is served as flat static files, verified (by the reviewer) to take precedence over the catch-all route for both flat files and nested `directory/index.html` paths. This is a pure relocation: no content edit, no route added, nothing gains the new nav shell. Phase B is what moves pages `public/` → `content/` one at a time to add the shell; this fix only restores the current no-shell behavior that existed before this branch. The 33 course directories (exact names, several containing spaces/`+`/`&`): `AI Developer`, `AP Physics 1`, `AP Physics 2`, `AP Physics C Electricity and Magnetism`, `AP Physics C Mechanics`, `AP STEM+`, `Advanced+ Courses`, `Algebra Geometry Fundamentals Review`, `Applications`, `Applied Machine Deep Learning`, `Career Applied Engineering`, `Cloud Computing A`, `Cloud Computing B`, `Computer Networking Fundamentals`, `Computer Programming 1`, `Computer Programming 2`, `Computer Programming 2+`, `Computer Programming Ethics`, `Data Handling CB`, `Differential Equations`, `Discrete Math`, `Engineering 1`, `Game Engine Architecture`, `Linear Algebra A`, `Mathematical Proofs`, `Multivariable Calculus`, `Pathways`, `Precalculus`, `Programming with Packages`, `Projects`, `Quantum Physics and Optics`, `Software Engineering`, `Systems Programming & Architecture`, `Video Game Modding`. The 22 root files: `advanced.html`, `applications.html`, `cpp-sandbox.html`, `developer.html`, `engineering.html`, `guided-language-project.html`, `java-sandbox.html`, `javascript-sandbox.html`, `login.html`, `math.html`, `pandas-sandbox.html`, `pathways.html`, `problem-set.html`, `problem-sets.html`, `projects.html`, `python-project.html`, `python-projects.html`, `python-sandbox.html`, `python-sensor-project.html`, `sandbox.html`, `science.html`, `technology.html`. Do not move `.agents/`, `.neon`, `skills-lock.json`, or any other repo-tooling/config file — those are unrelated to web serving.

**Fix 2 (Important):** `styles/globals.css`'s `@theme { }` block (currently lines 85–92) declares `--color-muted`, `--color-border`, `--color-accent` mapped to the correct `--site-*` tokens, but the `@theme inline` block below it (shadcn-generated) redeclares the same three keys mapped to shadcn's own `--muted`/`--border`/`--accent` (which point at `--site-accent-soft` for two of the three) — and since it comes second, it wins. `--color-border` happens to coincide in value today, but `--color-muted` and `--color-accent` resolve to the pale cream `--site-accent-soft` instead of the intended gray/brown. Nothing in the shipped code currently uses the affected Tailwind utility classes (`text-muted`, `text-accent`, `bg-accent-soft` — `Layout.tsx` uses arbitrary values like `text-[var(--site-accent)]` instead), so this is dead-but-misleading code, not a live bug. Fix: delete the `@theme { }` block (lines 85–92) entirely — smaller diff than renaming, and nothing depends on it.

**Fix 3 (Important):** Task 1 moved `assets/` → `public/assets/`, but 12 `scripts/check-*.js` files still `require('../assets/...')` at the old path, breaking the syntax-highlighting, code-editor, problem-bank, Java/JS/C++/Python sandbox, and guided-language-project checks (verified: `check-syntax-highlight.js`, `check-problem-banks.js`, `check-python-sandbox.js`, `check-code-editor.js` all throw `MODULE_NOT_FOUND`). These aren't wired into `npm test` today (only `check-auth-session`/`check-password`/`check-content` are), so nothing currently reports the breakage — but they're the only regression coverage for the ~15 interactive subsystems this migration promises not to disturb. Fix: change every `require('../assets/...')` to `require('../public/assets/...')` across `scripts/*.js`, plus the one non-`require` reference at `scripts/check-python-sandbox.js:85` (`path.join(root, 'assets/python-worker.js')` → `path.join(root, 'public/assets/python-worker.js')`). Exact list of affected files/lines: `check-syntax-highlight.js:3`, `check-java-sandbox.js:5`, `check-javascript-cpp-sandboxes.js:7-8,28`, `check-code-editor.js:3`, `check-pandas-sandbox.js:5`, `check-python-project.js:7-8`, `check-python-sandbox.js:6,85`, `check-guided-language-projects.js:7`, `check-problem-banks.js:5`. Do not add these scripts to `package.json`'s `test` script — that's a separate decision outside this fix's scope, not something the review required.

After all three fixes: re-run `rm -rf .next && npx next build` and `npm test` (still just the 3 wired checks) to confirm nothing broke, then spot-check a couple of the newly-relocated `public/` pages are actually reachable (e.g. `curl -sI http://localhost:3000/math.html` and one nested course page after `next start`).
